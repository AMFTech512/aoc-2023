...
fn main() {
    ...
    // this stores the absolute positions of all the symbols
    let mut symbol_set: HashSet<u32> = HashSet::new();
        ...
        match line {
            Ok(line) => {
                ...
                for local_symbol_loc in local_symbol_locs {
                    symbol_set.insert(local_symbol_loc + (idx * &line.len()) as u32);
                }
    ...
    for pnum in pnum_matches {
        // get the locations of the symbols in the line above and below
        let search_locs =
            get_search_locs(pnum.len, pnum.local_start, pnum.line_num, pnum.line_size);

        for search_loc in search_locs {
            // if the symbol is in the symbol set, we have a match
            if symbol_set.contains(&search_loc) {
